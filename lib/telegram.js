...
const PROFILE_TOKEN_ALPHABET = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
const PROFILE_TOKEN_LENGTH = 6;
...
const normalizeProfileToken = (token) =>
  (token || '')
    .toUpperCase()
    .replace(/[^A-Z0-9]/g, '');
...
const createProfileToken = async () => {
  while (true) {
    const bytes = crypto.randomBytes(PROFILE_TOKEN_LENGTH);
    const token = Array.from(bytes)
      .map((byte) => PROFILE_TOKEN_ALPHABET[byte % PROFILE_TOKEN_ALPHABET.length])
      .join('');

    const existing = await redisRequest(['GET', tgProfileKey(token)]);
    if (!existing) {
      return token;
    }
  }
...
export async function getTelegramUserByProfileToken(token) {
  const normalizedToken = normalizeProfileToken(token);
  if (!normalizedToken) return null;

  const chatId = await redisRequest(['GET', tgProfileKey(normalizedToken)]);
...
const ensureProfileToken = async (user) => {
  if (user.profile_token) {
    user.profile_token = normalizeProfileToken(user.profile_token);
    await redisRequest(['SET', tgProfileKey(user.profile_token), String(user.chat_id)]);
    return user.profile_token;
  }

  const profileToken = await createProfileToken();
...
export const getProfileTokenFromRequest = (request) =>
  normalizeProfileToken(request.cookies.get(PROFILE_COOKIE_NAME)?.value || null);

export const attachProfileCookie = (response, profileToken) => {
  response.cookies.set(PROFILE_COOKIE_NAME, normalizeProfileToken(profileToken), {
    httpOnly: true,
    sameSite: 'lax',
